  ...
  return (
    <>
      <button
        ...
        aria-label={isEvent ? 'Close event' : 'Close organisation'}
        onClick={onClose}
      />
      <aside
        className="task-drawer campus-drawer"
        role="dialog"
        aria-modal="true"
        aria-labelledby="campus-drawer-title"
        ref={panel}
      >
        <div className="drawer-header">
          <div className="drawer-label">
            <span>{item.category}</span>
            <span>{isEvent ? longDate(item.date) : item.meets}</span>
          </div>
